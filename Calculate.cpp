...
{
    return ((ch >= '0' && ch <= '9') || ch == '.' || ch == 'e') ? true : false;//添加科学计数法e
}
...
double Calculate_Cla::ToData(char *ch)   //将数字转化为数值
{
    ...
    {/////////
        if (i < strlen(ch))
            for (j = i + 1; j < strlen(ch); j++)        //小数部分，到e为止
            {
                if (ch[j] != 'e')
                    sum = sum * 10 + (ch[j] - '0');
                else break;
            }
        sum /= pow(10.0, (double) (strlen(ch) - 6 - i)); //有错
        if (ch[j] == 'e' && j < strlen(ch)) {
//    if(ch[j]!='\0')      //科学计数法部分e
            for (k = j + 2; k <= j + 4; k++)
                summ = summ * 10 + (ch[k] - '0');// ch[j]的ascii码减去0的ascii码
            if (ch[j + 1] == '-') {
                double p = (sum + sumn) / pow(10, summ);
                return ((sum + sumn) / pow(10, summ));                    //返回值
            }
            if (ch[j + 1] == '+') {
                double q = (sum + sumn) * pow(10, summ);
                return ((sum + sumn) * pow(10, summ));
            }
        }
    }/////////////////
